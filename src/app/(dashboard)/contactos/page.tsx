      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Directorio de Contactos</h1>
          </div>
              ...
              {contactos.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-12 text-center text-slate-400">
                    ...
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">No se encontraron contactos</p>
                    ...
                    <tr key={p.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition">
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-600/10 border border-blue-200 dark:border-blue-500/20 text-blue-700 dark:text-blue-400 flex items-center justify-center font-bold text-xs shrink-0">
                            {p.nombre.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-slate-900 dark:text-white">{p.nombre}</p>
                            <p className="text-[11px] text-slate-500 dark:text-slate-400">{p.cargo || "Sin cargo principal"}</p>
                          </div>
                        </div>
                      </td>
                        ...
                        <div className="space-y-1">
                          {p.email && (
                            <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
                              ...
                              <a href={`mailto:${p.email}`} className="hover:underline text-blue-600 dark:text-blue-400 font-medium">
                                {p.email}
                              </a>
                            </div>
                          )}
                          ...
                          {!p.email && !p.telefono && <span className="text-slate-400 italic">—</span>}
                        </div>
                      </td>

                      <td className="py-4 px-4">
                        {p.afiliaciones.length === 0 ? (
                          <span className="text-slate-400 italic">Sin empresa vinculada</span>
                        ) : (
                            ...
                            {p.afiliaciones.map((af) => (
                              <Link
                                ...
                                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-semibold border transition ${
                                  af.activa
                                    ? "bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:border-blue-500"
                                    : "bg-slate-100/50 dark:bg-slate-950/40 text-slate-400 border-slate-200 dark:border-slate-800 line-through"
                                }`}
                              >
                                <Building2 className="w-3 h-3 text-slate-400" />
                                <span>{af.cuenta.razonSocial}</span>
                                {af.cargo && <span className="text-slate-500 dark:text-slate-400 text-[10px]">({af.cargo})</span>}
                              </Link>
                            ))}
                          </div>
                        )}
