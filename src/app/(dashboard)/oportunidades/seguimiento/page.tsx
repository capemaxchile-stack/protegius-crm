...
export default async function SeguimientoPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-white tracking-tight">Seguimiento Comercial Diario</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2 text-rose-400">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-wider">Vencidas ({tareasVencidas.length})</h2>
            </div>
          ...
          {tareasVencidas.length === 0 ? (
            <p className="text-slate-500 text-xs italic text-center py-6">No hay tareas vencidas. ¡Excelente!</p>
          ) : (
            <div className="space-y-2.5">
              {tareasVencidas.map((t) => (
                <div
                  key={t.id}
                  className="p-3 rounded-xl bg-slate-950 border border-rose-900/30 text-xs space-y-1"
                >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2 text-amber-400">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-wider">Para Hoy ({tareasHoy.length})</h2>
            </div>
          ...
          {tareasHoy.length === 0 ? (
            <p className="text-slate-500 text-xs italic text-center py-6">No hay compromisos agendados para hoy.</p>
          ) : (
            <div className="space-y-2.5">
              {tareasHoy.map((t) => (
                <div
                  key={t.id}
                  className="p-3 rounded-xl bg-slate-950 border border-amber-900/30 text-xs space-y-1"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-white">{t.titulo}</span>
                    <span className="text-[10px] font-semibold text-amber-400">Hoy</span>
                  </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          ...
          {tareasProximas.length === 0 ? (
            <p className="text-slate-500 text-xs italic text-center py-6">Sin tareas para los próximos 7 días.</p>
          ) : (
            <div className="space-y-2.5">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
        ...
        <div className="space-y-3">
          {actividadesRecientes.map((act) => (
            <div
              key={act.id}
              className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3"
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 capitalize">
                    {act.tipo}
                  </span>
                  <Link
                  ...
                  {act.oportunidad && (
                    <span className="text-slate-400">
                      · Negocio: <strong className="text-slate-300">{act.oportunidad.nombre}</strong>
                    </span>
                  )}
                </div>
                <p className="text-slate-300 text-xs">{act.descripcion}</p>
              </div>

              <div className="text-right text-[11px] text-slate-500 shrink-0">
                <p>{formatearFecha(act.fechaRealizada)}</p>
                <p className="text-slate-400">{act.responsable || "Ejecutivo"}</p>
              </div>
            </div>
          ))}
        </div>
