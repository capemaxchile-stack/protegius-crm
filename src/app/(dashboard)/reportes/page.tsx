    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
          ...
          <p className="text-2xl font-bold text-purple-400 tracking-tight">{winRate}%</p>
          <p className="text-xs text-slate-400 mt-1">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
          ...
          <p className="text-2xl font-bold text-white tracking-tight">{actividades.length}</p>
          <p className="text-xs text-slate-400 mt-1">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl space-y-4 p-5">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          ...
          <span className="text-[11px] text-slate-400">{metricasPorEjecutivo.length} miembros activos</span>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800 text-slate-200">
              {metricasPorEjecutivo.map((m) => (
                <tr key={m.user.id} className="hover:bg-slate-800/30 transition">
                  <td className="py-3.5 px-4 font-semibold text-white">
                    {m.user.name}
                    <span className="block text-[10px] text-slate-400 font-normal">{m.user.email}</span>
                  </td>
                  ...
                  <td className="py-3.5 px-4 text-center text-slate-300">
                    <span className="text-blue-400 font-medium">{m.llamadas} llam.</span> ·{" "}
                    <span className="text-purple-400 font-medium">{m.reuniones} reun.</span>
                  </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            ...
            <span className="text-[11px] text-slate-400">Distribución de causas</span>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          ...
          {cuentasDesatendidas.length === 0 ? (
            <p className="text-emerald-400 text-xs text-center py-6 font-medium">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {cuentasDesatendidas.slice(0, 8).map((c) => (
                ...
                >
                  <div>
                    <p className="font-semibold text-white">{c.razonSocial}</p>
                    <p className="text-[10px] text-slate-400">
                      Rubro: {c.rubro || "General"} · Estado: {c.etapa}
                    </p>
                  </div>
                  <span className="text-[10px] text-amber-400 font-medium">Sin contacto</span>
                </div>
              ))}
