...
export default async function PropuestaDetallePage({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Emitida para: <strong>{propuesta.clienteRazonSocial}</strong>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {propuesta.estado === "borrador" && (
            ...
            >
              <button
                type="submit"
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-medium text-xs shadow-sm transition"
              >
          ...
          {!esAceptada && !esRechazada && (
            <>
              ...
              >
                <button
                  type="submit"
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 font-medium text-xs transition"
                >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-8 print:bg-white print:text-slate-900 print:border-none print:shadow-none print:p-0">
        {/* Document Header */}
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 pb-6 border-b border-slate-800 print:border-slate-300">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg text-white tracking-tight print:text-slate-900">
                PROTEGIUS
              </span>
            </div>
          ...
          <div className="sm:text-right">
            <span className="text-[11px] font-semibold text-blue-400 uppercase tracking-wider block print:text-blue-700">
              Propuesta Comercial
            </span>
            <p className="text-xl font-mono font-bold text-white mt-0.5 print:text-slate-900">
              N° PROP-{propuesta.numero}
            </p>
            <div className="text-xs text-slate-400 space-y-0.5 mt-2 print:text-slate-600">
        ...
        {/* Client Metadata */}
        <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 print:bg-slate-50 print:border-slate-200">
          <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 print:text-slate-700">
            Datos del Destinatario
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div>
              <span className="text-slate-500 block print:text-slate-500">Razón Social</span>
              <span className="font-bold text-white text-sm print:text-slate-900">
                {propuesta.clienteRazonSocial}
              </span>
            </div>
            <div>
              <span className="text-slate-500 block print:text-slate-500">RUT</span>
              <span className="font-mono text-slate-200 print:text-slate-800">
                {propuesta.clienteRut || "Sin registrar"}
              </span>
            </div>
            <div>
              <span className="text-slate-500 block print:text-slate-500">Giro / Rubro</span>
              <span className="text-slate-200 print:text-slate-800">
                {propuesta.clienteGiro || "General"}
              </span>
            </div>
            <div>
              <span className="text-slate-500 block print:text-slate-500">Atención a</span>
              <span className="text-slate-200 font-medium print:text-slate-800">
                {propuesta.clienteContacto || "Gerencia Comercial / Compras"}
              </span>
            </div>
            <div>
              <span className="text-slate-500 block print:text-slate-500">Correo Electrónico</span>
              <span className="text-slate-200 print:text-slate-800">
                {propuesta.clienteEmail || "—"}
              </span>
            </div>
            <div>
              <span className="text-slate-500 block print:text-slate-500">Teléfono</span>
              <span className="text-slate-200 print:text-slate-800">
        ...
        {propuesta.servicios.length > 0 && (
          <div className="space-y-3">
            <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider print:text-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {propuesta.servicios.map((serv) => (
                <div
                  key={serv.id}
                  className="p-3.5 rounded-xl bg-slate-950/40 border border-slate-800/80 text-xs space-y-1 print:bg-white print:border-slate-200"
                >
                  <div className="flex items-center gap-1.5">
                    <Shield className="w-3.5 h-3.5 text-blue-400 print:text-blue-600" />
                    <span className="font-semibold text-white print:text-slate-900">{serv.nombre}</span>
                  </div>
                  {serv.descripcion && (
                    <p className="text-slate-400 text-[11px] print:text-slate-600">{serv.descripcion}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="border border-slate-800 rounded-2xl overflow-hidden print:border-slate-300">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200 print:divide-slate-200 print:text-slate-800">
                {propuesta.planes.map((p) => (
                  <tr key={p.id}>
                    <td className="py-3.5 px-4 font-semibold text-white print:text-slate-900">
                      {p.planNombre}
                      <span className="block text-[10px] text-slate-400 font-normal print:text-slate-500">
                        {p.grupoNombre}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-400 print:text-slate-600">{p.detalle || "—"}</td>
                    <td className="py-3.5 px-4 text-right font-mono text-slate-400 print:text-slate-600">
                    ...
                    <td className="py-3.5 px-4 text-right font-bold text-white print:text-slate-900">
        ...
        <div className="flex flex-col sm:flex-row justify-end">
          <div className="w-full sm:w-80 bg-slate-950/80 border border-slate-800 rounded-2xl p-5 space-y-2.5 text-xs print:bg-slate-50 print:border-slate-300">
            <div className="flex justify-between text-slate-400 print:text-slate-600">
              <span>Subtotal Planes Mensual:</span>
              <span className="font-semibold text-white print:text-slate-900">{formatearUF(totalPlanesUF)}</span>
            </div>

            {setupFinalUF > 0 && (
              <div className="flex justify-between text-slate-400 print:text-slate-600">
                <span>Setup / Habilitación:</span>
                <span className="font-semibold text-white print:text-slate-900">{formatearUF(setupFinalUF)}</span>
              </div>
            )}
            ...
            <div className="pt-3 border-t border-slate-800 print:border-slate-300 flex justify-between items-baseline">
              ...
              <span className="font-black text-blue-400 text-lg print:text-blue-800">
                {formatearUF(totalConSetupUF)}
              </span>
            </div>

            <p className="text-[10px] text-slate-500 text-right print:text-slate-500">
        ...
        <div className="pt-6 border-t border-slate-800 space-y-6 text-xs text-slate-400 print:border-slate-300 print:text-slate-600">
          <div className="space-y-1.5">
            <h3 className="font-semibold text-slate-300 print:text-slate-800">Condiciones Comerciales:</h3>
          ...
          <div className="pt-12 grid grid-cols-2 gap-12 text-center text-xs">
            <div className="border-t border-slate-700 pt-2 print:border-slate-400">
              <p className="font-semibold text-slate-200 print:text-slate-900">Protegius SpA</p>
              <p className="text-[11px] text-slate-500">Firma Comercial Autorizada</p>
            </div>
            <div className="border-t border-slate-700 pt-2 print:border-slate-400">
              <p className="font-semibold text-slate-200 print:text-slate-900">{propuesta.clienteRazonSocial}</p>
